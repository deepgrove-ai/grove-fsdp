...
_allocator = None
_registered_pool_groups = set()
_constructor_symmetric_pools = set()


def _pool_group_key(pool, group):
    return (id(pool), id(group))


def _register_backend_mem_pool(backend, pool, group, symmetric):
    key = _pool_group_key(pool, group)
    if key in _registered_pool_groups:
        return
    if symmetric:
        if id(pool) in _constructor_symmetric_pools:
            backend.register_mem_pool(pool)
        else:
            backend.register_mem_pool(pool, symm=True)
    else:
        backend.register_mem_pool(pool)
    _registered_pool_groups.add(key)


def _deregister_backend_mem_pool(backend, pool, group):
    key = _pool_group_key(pool, group)
    if key not in _registered_pool_groups:
        return
    backend.deregister_mem_pool(pool)
    _registered_pool_groups.discard(key)
...
def create_nccl_mem_pool(symmetric=None):  # symmetric: bool | None = None -> torch.cuda.MemPool:
    ...
    if not is_torch_min_version("2.9.0a0") and symmetric is True:
        raise RuntimeError(
            "Symmetric memory pool is not supported with torch version < 2.9.0a0. "
    ...
    if not symmetric:
        ...
        if 'symmetric' in get_func_args(torch.cuda.MemPool):
            ...
            _constructor_symmetric_pools.add(id(_pool))
...
def register_mem_pool(pool, group, symmetric=True):
    ...
    _register_backend_mem_pool(backend, pool, group, symmetric)
...
def deregister_mem_pool(pool, group):
    ...
    backend = group._get_backend(torch.device("cuda", torch.cuda.current_device()))
    if pool.snapshot():
        _deregister_backend_mem_pool(backend, pool, group)
...
class nccl_mem:
        ...
        if self.group is not None:
            ...
            if self.pool.snapshot():
                ...
                try:
                    ...
                    # registered memory.
                    _deregister_backend_mem_pool(backend, self.pool, self.group)
                except RuntimeError:
    ...
    def __exit__(self, *args):
        try:
            if self.group is not None:
                backend = self.group._get_backend(self.device)
                _register_backend_mem_pool(backend, self.pool, self.group, self.symmetric)
        finally:
...
class MultiGroupMemPoolAllocator:
        ...
        if self.pool.snapshot():
            for group in self.groups:
                ...
                try:
                    ...
                    _deregister_backend_mem_pool(backend, self.pool, group)
                except RuntimeError:
    ...
    def __exit__(self, *args):
        try:
            for group in self.groups:
                backend = group._get_backend(torch.device("cuda", torch.cuda.current_device()))
                _register_backend_mem_pool(backend, self.pool, group, self.symmetric)
        finally:
            self.mem_context.__exit__(*args)
