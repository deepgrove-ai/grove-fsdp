...
class GroveFSDP(torch.nn.Module):
    ...
    Args:
        ...
        dist_index (FSDPDistributedIndex): FSDPDistributedIndex object containing references to the
            process groups and device meshes used by Grove-FSDP.
        ...
        keep_fp8_transpose_cache (bool): Whether to keep
            the fp8 transpose cache when using Grove-FSDP. It will use significantly
    ...
    def __init__(
        ...
        if self.device != torch.device(f"cuda:{torch.cuda.current_device()}"):
            logger.warning(
                f"[Rank {torch.distributed.get_rank()}] Grove-FSDP is "
                ...
                "send all Tensors in the module to the Grove-FSDP "
        ...
        # otherwise Grove-FSDP will proceed with the existing module and send the model
        ...
        # FSDPDistributedIndex stores the process groups and meshes used by Grove-FSDP.
        # If not provided, Grove-FSDP will default to a simple data parallel index
        ...
        if (
            ...
            raise ValueError(
                "[Grove-FSDP] Megatron Expert Parallelism is enabled, but no expt_dp_group is"
                "provided."
            )
        ...
        # accessibility, e.g. when attaching GroveFSDP scheduled ops
    ...
    def sync(self):
        """
        Context manager that synchronizes the GroveFSDP model parameters and gradients
    ...
    def set_model_auto_sync(self, sync_model: bool = True):
        """
        Activate or deactivate flag that controls Grove-FSDP model synchronization.
        ...
        Otherwise, GroveFSDP will perform such synchronizations every optimization
        ...
        GroveFSDP.model_auto_sync = True.

        Args:
            sync_model (bool, optional): Whether to synchronize the model every training step.
                GroveFSDP.model_auto_sync will be set to the value of sync_model.
                Defaults to True. GroveFSDP.model_auto_sync defaults to False.
        """
        if sync_model and (
            ...
            log_single_rank(
                ...
                msg="[Grove-FSDP.set_model_auto_sync()] Detected Grove-FSDP "
                "auto-synchronization with the 'no_shard' / 'optim' sharding "
                "strategy for either FSDP (DP-Shard) or HSDP (DP-Outer). "
                "`GroveFSDP.zero_grad_buffer()` or `optimizer.zero_grad()` "
    ...
    def get_distributed_index(self) -> FSDPDistributedIndex:
        """
        Get the distributed environment of Grove-FSDP, which contains references
    ...
    def mixed_precision_context(self, mixed_precision_policy: MixedPrecisionPolicy):
        ...
        for GroveFSDP / ParamAndGradBuffer.
        """
    ...
    def reset_mixed_precision_policy(self, mixed_precision_policy: MixedPrecisionPolicy):
        """
        Re-configure MixedPrecisionPolicy for GroveFSDP / ParamAndGradBuffer.
        """
    ...
    def _replace_param_with_distributed_if_needed(self):
        ...
        for name, _ in self.module.named_parameters():
            ...
            # DTensor parameter is managed by Grove FSDP.
    ...
    def install_optimized_model_weights(self):
        ...
        managed by Grove-FSDP. Should be called after the optimizer.step().
        """
